...
> **🎉 ВЕРСИЯ 3.0 - Двухэтапное развертывание с предварительной генерацией sudoers для IDM**
...
**🆕 НОВОЕ в v3.0:**
- ✅ Двухэтапный процесс развертывания (PREPARE → DEPLOY)
- ✅ Предварительная генерация sudoers с SHA256 хешами для IDM
- ✅ Скрипты-обертки с whitelist валидацией
- ✅ Полное соответствие замечаниям кибербезопасности

## ⚡ Quick Start (v3.0)

**Для нетерпеливых - двухэтапное развертывание за 55 минут:**

```bash
# Этап 1: PREPARE (генерация sudoers)
cd secure_deployment/sudoers
./generate_sudoers_with_hashes.sh CI04523276_CI10742292

# → Развернуть scripts на сервер
# → Создать 3 заявки в IDM
# → Дождаться согласования (1-3 дня)

# Этап 2: DEPLOY (через Jenkins)
# STAGE_MODE=deploy → полное развертывание
```

**Детальная инструкция:** [QUICKSTART_SUDOERS_GENERATION.md](QUICKSTART_SUDOERS_GENERATION.md)
...
✅ **Автоматизация**
- Jenkins Pipeline для CI/CD (двухэтапный процесс)
- Ansible playbooks для управления конфигурацией
- Интеграция с RLM для установки ПО
- Vault Agent для управления секретами
- 🆕 Автоматическое извлечение KAE_STEND из NAMESPACE_CI
- 🆕 Автоматическая генерация sudoers с SHA256 хешами

✅ **Безопасность**
- Явные права sudo без переменных и звездочек
- 🆕 Скрипты-обертки с SHA256 хешами для защиты от подмены
- 🆕 Whitelist валидация всех входных параметров
- Автоматическая ротация секретов через Vault Agent
- Защита от эскалации привилегий (NOEXEC флаг)
- Аудит всех операций
...
secure_deployment/
├── README.md                              # Этот файл
├── 🆕 README_IMPLEMENTATION_COMPLETE.md   # Итоги реализации v3.0
├── 🆕 QUICKSTART_SUDOERS_GENERATION.md    # Быстрый старт (55 мин)
├── docs/                                  # Документация
...
│   ├── KAE_STEND_NAMING.md                 # Динамические имена пользователей
│   ├── DYNAMIC_GROUP_GUIDE.md              # Динамическая группа (группа = СУЗ)
│   ├── 🆕 SUDOERS_GENERATION_PROCESS.md    # Двухэтапный процесс с sudoers
│   ├── 🆕 QUESTIONS_FOR_CYBERSECURITY.md   # Вопросы для согласования
│   └── 🆕 CORPORATE_SECURITY_RULES.md      # Правила безопасности + реальные кейсы
...
│   └── 🆕 wrappers/                        # Скрипты-обертки с валидацией
│       ├── manage_firewall.sh              # Управление iptables (whitelist)
│       ├── manage_vault_certs.sh           # Обработка сертификатов
│       └── call_rlm_api.sh                 # Вызовы RLM API
...
│   ├── monitoring_ci                           # Права для ТУЗ (CI/CD) - legacy
│   ├── monitoring_admin                        # Права для ПУЗ (Admin) - legacy
│   ├── monitoring_ro                           # Права для ReadOnly - legacy
│   ├── 🆕 monitoring_ci_wrappers.template      # Template для ТУЗ с SHA256
│   ├── 🆕 monitoring_admin_wrappers.template   # Template для ПУЗ с SHA256
│   ├── 🆕 monitoring_ro_wrappers.template      # Template для RO с SHA256
│   ├── 🆕 generate_sudoers_with_hashes.sh      # Генератор с SHA256 (v3.0)
│   ├── generate_sudoers.sh                     # Генератор для динамических имен
│   └── README_DYNAMIC_USERS.md                 # Инструкция по динамическим именам
└── Jenkinsfile                        # Jenkins Pipeline

```

## 🆕 Динамические имена пользователей и групп

Начиная с версии 2.0, проект поддерживает **автоматическое формирование имен пользователей и групп** на основе **KAE стенда**.
...
NAMESPACE_CI = "CI04523276_CI10742292"
            ...
            KAE_STEND = "CI10742292"
                          ↓
Пользователи:
- CI10742292-lnx-mon_sys      (СУЗ) → primary group: CI10742292-lnx-mon_sys
- CI10742292-lnx-mon_admin    (ПУЗ) → secondary group: CI10742292-lnx-mon_sys
- CI10742292-lnx-mon_ci       (ТУЗ) → secondary group: CI10742292-lnx-mon_sys
- CI10742292-lnx-mon_ro       (ReadOnly) → secondary group: CI10742292-lnx-mon_sys

Группа для стенда:
- CI10742292-lnx-mon_sys (создается автоматически при создании СУЗ в IDM)
```

### Преимущества

✅ **Уникальность** - каждый стенд имеет свои УЗ и группу  
✅ **Прослеживаемость** - имя содержит KAE стенда  
✅ **Автоматизация** - Jenkins извлекает имена, IDM создает группу  
✅ **Масштабируемость** - легко развертывать множество стендов  
✅ **Изоляция** - полная изоляция между стендами через группы  

**📖 Подробнее**:
- [KAE_STEND_NAMING.md](docs/KAE_STEND_NAMING.md) - динамические имена пользователей
- [DYNAMIC_GROUP_GUIDE.md](docs/DYNAMIC_GROUP_GUIDE.md) - динамическая группа (группа = СУЗ)

---

## Быстрый старт

### Выбор схемы именования

**Вариант А: Статические имена** (для одного стенда)
- `monitoring_svc`, `monitoring_admin`, `monitoring_ci`, `monitoring_ro`

**Вариант Б: Динамические имена** (рекомендуется для множественных стендов)
- `{KAE_STEND}-lnx-mon_sys`, `{KAE_STEND}-lnx-mon_admin`, и т.д.
...
6. 🆕 **NAMESPACE_CI** - должен содержать разделитель '_' (например: `CI04523276_CI10742292`)
